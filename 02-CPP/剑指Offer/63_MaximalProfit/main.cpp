...
/**
 面试题63:股票的最大利润
 题目：假设把某股票的价格按照时间先后顺序存储在数组中，请问卖该股票一次可能的最大利润是多少？例如，一只股票在某些时间节点
 的价格为 {9, 11, 8, 5, 7, 12, 16, 14}。如果我们能在价格为5的时候买入并在价格为16时卖出，则能获得最大利润为11。
 */

/**
 分析：
 蛮力法：对数组中所有的2个元素的组合求差值。T = O(n^2)
 方法二：T = O(n)
 */
