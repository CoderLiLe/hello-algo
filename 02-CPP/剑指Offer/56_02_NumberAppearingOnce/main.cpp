...
/**
 面试题56:数组中数字出现的次数
 题目二：数组中唯一只出现一次的数字。
    在一个数组中除一个数字只出现一次外，其他数字都出现了三次。请找出那个只出现了一次的数字。
 */

/**
 分析：
 如果一个数字出现三次，那么它的二进制表示的每一位（0或者1）也出现三次。如果把所有出现三次的数字的二进制位
 表示的每一位都分别加起来，那么每一位的和都能被3整除。T = O(n), S = O(1)
 
 数组排序法：T = O(nlogn)
 哈希表来记录每个数字出现的次数，S = O(n)
 */
